...
//patch is used to update a piece of data
app.patch("/student/:_id", async (req, res) => {
  const { _id } = req.params;
  let { name, age, email, mobile } = req.body;
...
const updateSpecificData = await student.save();

  res.json({
    success: true,
    data: updateSpecificData,
    message: `Update Specific Data Successfuly With Id ${_id}`,
  });
});
